...
int main(int argc, char *argv[]){
  ...
  for (int i = 0; i < NUM_TESTS; i++){
    ...
    int draw_total = 4;     // Exactly 1 card should be drawn from deck
    int draw_total_other = 1; // Each other player should draw 1 card
    ...
    randomCards(G.hand[currentPlayer], G.handCount[currentPlayer]); // randomize the hand itself
    cr_position = randomRange(0, G.handCount[currentPlayer] - 1); // randomize position of council room
    G.hand[currentPlayer][cr_position] = council_room;  // place council room in hand
    ...
    dep.dep_names[2] = "number of buys";
    dep.dependencies[0] = G.handCount[currentPlayer];
    dep.dependencies[1] = cr_position;
    dep.dependencies[2] = G.numBuys;

    // Save game state & execute via cardEffect
    memcpy(&testG, &G, sizeof(struct gameState));
    cardEffect(council_room, choice1, choice2, choice3, &testG, cr_position, &bonus);
    ...
    assertEqualValPair(testG.handCount[otherPlayer], G.handCount[otherPlayer] + draw_total_other, hand_count_other, &hand_count_other_errors, dep);
    assertEqualValPair(testG.deckCount[otherPlayer], G.deckCount[otherPlayer] - draw_total_other, deck_count_other, &deck_count_other_errors, dep);
